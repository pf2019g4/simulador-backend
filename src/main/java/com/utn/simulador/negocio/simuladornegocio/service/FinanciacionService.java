...
public class FinanciacionService {
    ...
    public void acreditar(Long proyectoId, boolean esForecast) {
        ...
        if (credito != null && credito.getMonto() != null && credito.getMonto().compareTo(BigDecimal.ZERO) > 0) {
            ...
            Cuenta cuentaFinancieraIngresoCredito = cuentaService.crearCuentaFinanciera(proyectoId, "Crédito", TipoFlujoFondo.INGRESOS_NO_AFECTOS_A_IMPUESTOS, null, TipoTransaccion.OTROS, esForecast);
            cuentaService.crearCuentaFinancieraPeriodo(periodoInicial, credito.getMonto(), cuentaFinancieraIngresoCredito);

            Cuenta cuentaFinancieraInteresCredito = cuentaService.crearCuentaFinanciera(proyectoId, "Interés deuda", TipoFlujoFondo.EGRESOS_AFECTOS_A_IMPUESTOS,TipoBalance.DEUDA_BANCARIA, TipoTransaccion.OTROS, esForecast);
            Cuenta cuentaFinancieraAmortCuotaCredito = cuentaService.crearCuentaFinanciera(proyectoId, "Amortización cuota", TipoFlujoFondo.EGRESOS_NO_AFECTOS_A_IMPUESTOS,TipoBalance.DEUDA_BANCARIA, TipoTransaccion.OTROS, esForecast);
